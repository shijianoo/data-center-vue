...
const BASE_URL = import.meta.env.DEV
  ? "http://150.158.28.39:9204/tianjin/query" // 开发环境
  : "http://150.158.28.39:9204/tianjin/query" // 生产环境，修改为实际地址

const client = axios.create({ baseURL: BASE_URL })

// 获取岸基站水文与气象数据
export function getShoreMeteoPageData(sn: string, page: number, pageSize: number) {
  return client.get(`shore/meteo/page?sn=${sn}&page=${page}&pageSize=${pageSize}`)
}
