    ...
    <table class="custom-table">
      ...
      <tbody>
        <tr @click="emit('action', row)" v-for="(row, rIdx) in devices" :key="rIdx" :class="{ 'bg-danger-light': row.isOnline === false }">
          ...
          <td>{{ latestsMap[row.serialNumber]?.temp_wat || "-" }}</td>
          <td>{{ latestsMap[row.serialNumber]?.humid || "-" }}</td>
          <td>{{ latestsMap[row.serialNumber]?.lon || "-" }}</td>
          <td>{{ latestsMap[row.serialNumber]?.lat || "-" }}</td>
