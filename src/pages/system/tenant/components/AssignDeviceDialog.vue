...
import {
  addDevicesToTenant,
  getDeviceIdsFromTenant,
  removeDevicesFromTenant
...
async function fetchData() {
  ...
  loading.value = true
  try {
    const [devicesRes, modelsRes, assignedIdsRes, tenantRes] = await Promise.all([
      getDevicesApi(),
      getDeviceModelsApi(),
      getDeviceIdsFromTenant(props.tenantId),
      getTenantApi(props.tenantId)
    ])
    allDevices.value = devicesRes.data || []
    allModels.value = modelsRes.data || []
    assignedDeviceIds.value = assignedIdsRes.data || []
    tenant.value = tenantRes.data || null
  } catch (error) {
...
async function handleRemove(deviceOrIds: Device | string[]) {
  ...
  const message = ids.length === 1
    ? `确定要从该租户中移除设备 ${allDevices.value.find(d => d.id === ids[0])?.serialNumber} 吗？`
    : `确定要从该租户中移除选中的 ${ids.length} 个设备吗？`
